...
CREATE
DEFINER=`root`@`localhost`
TRIGGER `payment`.`update_denomination_after_update`
...
BEGIN
    IF NEW.st_withdrawal = 1 THEN
        UPDATE denomination
        SET d_quantity = CASE
            WHEN d_quantity > 0 OR d_quantity = 0 THEN d_quantity + (NEW.st_quantity - OLD.st_quantity)
            ELSE d_quantity
		END
        WHERE d_denom_value = NEW.st_denom_value
        AND d_empid = NEW.st_empid
    ...
    ELSEIF NEW.st_withdrawal = 0 THEN
        UPDATE denomination
         SET d_quantity = CASE
            WHEN d_quantity > 0 THEN d_quantity - (NEW.st_quantity - OLD.st_quantity)
            ELSE d_quantity
		END
        WHERE d_denom_value = NEW.st_denom_value
        AND d_empid = NEW.st_empid
